...
def index():
    ...
    return render_template("index.html", items=items,
                           tool_type=tool_type, serial_number=serial_number,
                           size=size, status=status, location=location)
...
def upload_excel():
    ...
    for col in required_columns:
        if col not in df.columns:
            return f"خطا: ستون {col} در فایل اکسل وجود ندارد.", 400
    ...
    for _, row in df.iterrows():
        cursor.execute("""
            INSERT INTO inventory (tool_type, serial_number, size, thread_type, location, status, report_link, description)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)""",
                       (row['tool_type'], row['serial_number'], row['size'], row['thread_type'], row['location'], row['status'], "", ""))
    conn.commit()
    conn.close()
    return redirect(url_for("index"))
